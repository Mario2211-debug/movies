...
import { Play, Film, Trash2, Edit } from "lucide-react";
...
export default function AllMovies({ onEditMovie }: AllMoviesProps) {
    ...
    <>
      ...
      <section className="flex flex-col items-center justify-center text-center py-16 px-4 md:px-20">
        ...
        <p className="text-gray-400 text-sm md:text-base max-w-2xl">
          Gerencie sua coleção completa de filmes. Visualize, edite e organize todos os seus filmes favoritos.
        </p>
      </section>
            ...
            <div className="relative p-4 flex flex-col justify-between h-64">
              <div className="flex justify-between items-start mb-2">
                <div className="flex gap-1">
                  {movie.watched && (
                    <div className="w-2 h-2 bg-green-400 rounded-full" title="Assistido" />
                  )}
                  {movie.rating > 0 && (
                    <div className="text-yellow-400 text-xs" title={`${movie.rating}/5 estrelas`}>
                      ⭐{movie.rating}
                    </div>
                  )}
                </div>
      ...
      {movies.length === 0 && (
        <div className="text-center py-16">
          ...
          <h3 className="text-xl font-semibold text-gray-400 mb-2">Nenhum filme encontrado</h3>
          <p className="text-gray-500">Adicione alguns filmes para começar sua coleção!</p>
        </div>
      )}
    </>
  );

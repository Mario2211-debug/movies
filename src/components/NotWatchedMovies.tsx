...
import { Play, Film, Trash2, Edit, Eye } from "lucide-react";
import { useMovie } from "@/context/useMovie";
import { useState } from "react";

interface NotWatchedMoviesProps {
  onEditMovie: (movie: any) => void;
}

export default function NotWatchedMovies({ onEditMovie }: NotWatchedMoviesProps) {
    ...
    <>
      ...
      <section className="flex flex-col items-center justify-center text-center py-16 px-4 md:px-20">
        ...
        <p className="text-gray-400 text-sm md:text-base max-w-2xl">
          Sua lista de desejos cinematográficos. Descubra novos mundos esperando por você.
        </p>
      </section>
      ...
      <section className="grid grid-cols-2 md:grid-cols-4 gap-6 px-6 md:px-12 pb-16">
        {movies.map((movie) => (
          <motion.div
            ...
            className={`relative rounded-xl overflow-hidden shadow-lg blur-cover p-1 border border-orange-500/20`}
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
      ...
      {movies.length === 0 && (
        <div className="text-center py-16">
          ...
          <h3 className="text-xl font-semibold text-gray-400 mb-2">Lista vazia</h3>
          <p className="text-gray-500">Adicione alguns filmes para criar sua lista de desejos!</p>
        </div>
      )}
    </>
  );
